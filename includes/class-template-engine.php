...
final class Template_Engine {
	...
	public function email_document( string $subject, string $body ): string {
		...
		return '<!doctype html><html dir="rtl"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width"></head>'
			...
			. '<tr><td style="padding:24px 28px;background:' . esc_attr( $primary ) . ';color:#fff"><table role="presentation" width="100%"><tr><td>' . $mark . '</td><td align="left" style="color:#f2cf91;font-size:12px;font-weight:bold">' . esc_html( $brand ) . '</td></tr></table></td></tr>'
	...
	private function fields_table( array $form, array $values ): string {
		...
		foreach ( $values as $key => $value ) {
			$text  = is_array( $value ) ? implode( '، ', $value ) : (string) $value;
			$html .= '<tr><th align="right" style="width:34%;background:#faf8f3;border-bottom:1px solid #eee9df;padding:11px 13px;color:#61706d;font-size:12px">' . esc_html( $labels[ $key ] ?? $key ) . '</th>'
